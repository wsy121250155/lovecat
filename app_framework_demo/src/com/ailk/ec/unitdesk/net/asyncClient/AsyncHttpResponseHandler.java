...
package com.ailk.ec.unitdesk.net.asyncClient;
...
public abstract class AsyncHttpResponseHandler implements
	...
	protected static final int LOST_MESSAGE = 6;
	...
	static class ResponderHandler extends Handler {
		...
		// private final SoftReference<AsyncHttpResponseHandler> mResponder;
		private final AsyncHttpResponseHandler mResponderHandler;

		ResponderHandler(AsyncHttpResponseHandler service) {
			// mResponder = new
			// WeakReference<AsyncHttpResponseHandler>(service);
			// mResponder = new
			// SoftReference<AsyncHttpResponseHandler>(service);
			mResponderHandler = service;
		}

		@Override
		public void handleMessage(Message msg) {
			// AsyncHttpResponseHandler service = mResponder.get();
			AsyncHttpResponseHandler service = mResponderHandler;
			if (service != null) {
	 ...
	 */
	public void onProgress(int bytesWritten, int totalSize) {
		Log.d(TAG, String.format("Progress %d from %d (%d)", bytesWritten,
				totalSize, (bytesWritten * 100) / totalSize));
	}
